...
import { ImageOff, Package, Check, Copy, Link2 } from 'lucide-react';
...
export default function ProposalsPage() {
  ...
  return (
    <div className="max-w-2xl space-y-4 pb-24">
      ...
      <p className="text-sm text-ink-400 -mt-3">Arma el box como si fueras el cliente, y genera un link para compartirlo.</p>
      ...
      {/* ---- Tarjeta resumen: foto + resumen + botón crear (sección 19) ---- */}
      ...
      <Section title={`Propuestas existentes (${proposals.length})`}>
        <div className="space-y-2">
          ...
          {proposals.map((p) => (
            <div key={p._id} className="flex justify-between items-center text-sm border-b border-rose-50 last:border-0 pb-2">
    ...
    <div className="grid grid-cols-2 gap-2.5">
      {items.map((item) => {
        ...
        return (
          <div
            key={item._id}
            className={`flex items-center gap-2.5 rounded-2xl p-2 border-2 transition-all ${
              active ? 'border-rose-600 bg-rose-25' : 'border-transparent bg-white shadow-card'
            }`}
          >
            <div className="w-10 h-10 rounded-lg bg-rose-50 shrink-0 overflow-hidden flex items-center justify-center p-1">
              {item.photoUrl ? (
                <img src={item.photoUrl} alt="" className="w-full h-full object-cover rounded-md" />
              ) : (
                <ImageOff className="w-3.5 h-3.5 text-rose-200" strokeWidth={1.5} />
              )}
            </div>
            <span className="text-sm font-medium text-ink-900 flex-1 leading-tight">{item.name}</span>
            <div className="flex items-center gap-1.5 shrink-0">
              <button className="w-6 h-6 bg-gray-100 rounded-full text-ink-600 text-xs font-semibold" onClick={() => onDelta(item, -1)}>
                −
              </button>
              <span className="text-sm font-semibold w-4 text-center tabular-nums">{qty}</span>
              <button className="w-6 h-6 bg-rose-600 text-white rounded-full text-xs font-semibold" onClick={() => onDelta(item, 1)}>
                +
              </button>
            </div>
          </div>
